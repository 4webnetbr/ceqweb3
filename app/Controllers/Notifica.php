...
class Notifica extends BaseController
{
    ...
    public function gravaNotifica($controler, $usuario, $registro, $msg, $tipo)
    {
        ...
        log_message('info', 'Controller Notifica ' . $controler);
        // debug($userorig);
        $control = str_contains($controler, '\/')
            ? substr(strrchr($controler, '\/'), 1)
            : $controler;
        $control = str_contains($control, '\\')
            ? substr(strrchr($control, '\\'), 1)
            : $control;

        $usuariospermissoes = $this->modpermis->getPermissaoTelaUsuario(false, false, false, $control);
        // debug($usuariospermissoes);
        log_message('info', 'Usuários Permissões ' . json_encode($usuariospermissoes));
        if (count($usuariospermissoes) > 0) {
            $texto = $msg . ' em ' . data_br(date('Y-m-d H:i:s')) . ' por: ' . $userorig;
            log_message('info', 'Msg Notifica ' . $texto);
            for ($up = 0; $up < count($usuariospermissoes); $up++) {
                ...
                log_message('info', 'Usuário destino ' . $usu_dest . ' - Permissão: ' . $permissoes);
                if ($usu_dest != $usuario && str_contains($permissoes, 'N')) {
                    ...
                    log_message('info', 'Inseriu notificação');
