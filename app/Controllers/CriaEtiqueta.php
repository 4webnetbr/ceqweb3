...
private function renderConteudoInterno(array $dados, array $camp, bool $exibirBorda = true)
{
    ...
    public function imprimeEtiqueta($etq_id)
    {
        ...
        // Converte PNG → ZPL usando a API Labelary
        $ch = curl_init('http://api.labelary.com/v1/printers/8dpmm/labels/4x6/0/');
        ...
        if ($httpCode != 200 || !$zpl) {
            return $this->response->setJSON(['erro' => 'Falha ao converter imagem para ZPL.']);
        }

        // Envia ZPL via socket para impressora Zebra
