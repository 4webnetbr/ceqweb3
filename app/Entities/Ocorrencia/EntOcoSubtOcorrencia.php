...
class EntOcoSubtOcorrencia extends Entity
{
    ...
    /**
     * Campos de uma linha da aba "Ações" — usado tanto na configuração do
     * subtipo (OcoSubtOcorrencia) quanto na finalização da tratativa
     * (OcoTrataOcorrencia::finalizar()), pois é a mesma coisa: uma ação
     * (tpa_id) com seus dados de execução (tmo_id/mod_id/tel_id/stt_id) e a
     * flag de finalização automática (sta_fina).
     *
     * @param bool        $somenteLeitura              tmo_id/mod_id/tel_id/stt_id ficam bloqueados (ações já configuradas no subtipo, na tratativa)
     * @param bool        $tpaIdLeitura                tpa_id fica bloqueado (catálogo fixo); false permite escolha livre (ação adicionada manualmente na tratativa)
     * @param string|null $urlAddCampo                 URL do botão "+"; default mantém o comportamento atual (OcoSubtOcorrencia)
     * @param bool        $bloqueiaExclusaoAutomatica   RN — ações de finalização automática (sta_fina='S') já configuradas não podem ser excluídas na tratativa
     */
    public function defCamposAcao(
        $dados = false,
        $pos = 0,
        $total = 1,
        $modo = 'edit',
        bool $somenteLeitura = false,
        bool $tpaIdLeitura = true,
        ?string $urlAddCampo = null,
        bool $bloqueiaExclusaoAutomatica = false
    ) {
        $dados = (array) $dados;
        ...
        $staFina->leitura       = $somenteLeitura;
        ...
        $add->place     = "Adicionar Campo";
        $add->classep   = "btn-outline-success btn-sm bt-repete";
        $add->funcChan  = "addCampo('" . ($urlAddCampo ?? base_url("OcoSubtOcorrencia/addCampoTp/")) . "','acoes',this)";
        $ret['bt_addtp']   = $add->crBotao();

        // RN — ações de finalização automática (sta_fina='S') já configuradas
        // não podem ser excluídas na tela de tratativa (são obrigatórias e
        // serão sempre executadas ao finalizar).
        if ($bloqueiaExclusaoAutomatica && ($dados['sta_fina'] ?? 'N') === 'S') {
            $ret['bt_deltp'] = '';
        } else {
            $del            = new MyCampo();
            $del->attrdata  = $atrib;
