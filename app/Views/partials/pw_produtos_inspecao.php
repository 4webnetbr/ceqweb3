...
<div class="accordion mb-0" id="accProdutos">
    <div class="accordion-item">
        ...
        <div id="collapseprod<?php echo $produtos[0]['req_id'] ?? '' ?>" class="accordion-collapse collapse show" aria-labelledby="headprod<?php echo $produtos[0]['req_id'] ?? '' ?>" data-bs-parent="#accProdutos">
            <div class="accordion-body p-0" style="max-height:<?php echo $maxHeig; ?>; height:auto; border-top: 1px solid white;">
                <table class="display compact table table-sm table-info table-striped table-hover table-vertical-borders col-12 no-footer dataTable tabela-pequena" aria-describedby="table_info">
                    <thead class="table-default bg-table-blue-dark col-12 overflow-x-auto">
                        <tr class=' w-100' style='min-height:39px; height:39px;'>
                            ...
                            <th class="sticky-top text-center align-middle text-wrap">Qtde.<br>Requisitada</th>
                            <th class="sticky-top text-center align-middle text-wrap">Qtde.<br>Atendida</th>
                            <th class="sticky-top text-center align-middle text-wrap">Data<br>Atendimento</th>
                            <th class="sticky-top text-center align-middle text-wrap">Qtde.<br>Conferida</th>
                            <th class="sticky-top text-center align-middle text-wrap">Data<br>Conferência</th>
                            <th class="sticky-top text-center align-middle text-wrap">Qtde<br>Aprovada</th>
                            <?
                            if (!$show) { ?>
                                <th class="sticky-top text-center align-middle text-wrap">Conformidade</th>
                            <? } ?>
                        </tr>
                    </thead>
                    <tbody style="max-height: 45vh; overflow-y: auto;">
                        ...
                        ?>
                            <tr class='linha_produto <?php echo ($zeb % 2 == 0) ? 'even' : 'odd'; ?>' id='<?php echo $produto["rep_id"] ?>'>
                                ...
                                <td class='text-start'>
                                    <?php echo $produto['pro_despro'] ?>
                                </td>
                                <td id='<?php echo $produto['pro_codbar_fabricante'] ?>' data-codbar='<?php echo $produto['pro_codbar_fabricante'] ?>' data-id='cbFab' class='text-start'>
                                    <?php echo $produto['fab_apeFab'] ?>
                                </td>
                                <td data-codbar='<?php echo $produto['lot_codbar'] ?>' data-id='cbLot' class='text-start'><?php echo $produto['lot_lote'] . '<br>' . data_br($produto['lot_validade']) ?></td>
                                <td id='req_<?php echo $produto["rep_id"] ?>'><?php echo $produto['rep_quantia'] ?></td>
                                <td id='ate_<?php echo $produto["rep_id"] ?>'><?php echo $produto['rpa_atendida'] ?></td>
                                <td id='dat_<?php echo $produto["rep_id"] ?>'><?php echo $produto['rpa_data'] ?></td>
                                <td id='cfe_<?php echo $produto["rep_id"] ?>'><?php echo $produto['rpa_conferida'] ?></td>
                                <td id='dcf_<?php echo $produto["rep_id"] ?>'><?php echo $produto['rpa_data_conferencia'] ?></td>
                                <td id='apr_<?php echo $produto["rep_id"] ?>'><?php echo $produto['rpa_aprovada'] ?></td>
                                <? if (!$show) { ?>
                                    <td>
                                        <?php echo $produto['bt_insvis']; ?>
                                        <?php echo $produto['bt_ok']; ?>
                                    </td>
                                <? } ?>
                            </tr>
                        <?php $zeb++;
