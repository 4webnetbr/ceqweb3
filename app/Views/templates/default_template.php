...
<head>
  ...
  <title><?php echo esc($title); ?> - <?php echo esc(NAME_APP); ?></title>
  ...
  <!-- Scripts principais -->
  <script src="<?php echo base_url('assets/jscript/jquery-3.6.3.js'); ?>"></script>
  ...
  */
  ...
  <!-- <script src="<? //=base_url('assets/fontawesome/js/all.min.js');
                    ?>" defer></script> -->
  <script src="<?php echo base_url('assets/jscript/my_default.js'); ?>"></script>
  <script src="<?php echo base_url('assets/jscript/my_menu.js'); ?>"></script>
  <script src="<?php echo base_url('assets/jscript/my_wsconn.js'); ?>"></script>

  <!-- Scripts adicionais (dinâmicos) -->
  <?php if (! empty($scripts)): ?>
    <?php foreach (explode(',', $scripts) as $script): ?>
      <script src="<?php echo base_url("assets/jscript/{$script}.js"); ?>"></script>
    <?php endforeach; ?>
  <?php endif; ?>
</head>
...
<body>
  ...
  <div id="bloqueiaTela">
    ...
    <div class="msgprocessando bg-info px-5 py-3 border border-1 border-dark rounded-pill text-center">
  ...
  <!-- Seções do layout -->
  <section class="header"><?php echo $this->renderSection('titulo'); ?></section>
  <section class="menu"><?php echo $this->renderSection('menu'); ?></section>
  <section class="content"><?php echo $this->renderSection('content'); ?></section>
  <section class="manutencao"><?php echo $this->renderSection('manutencao'); ?></section>
  <section class="footer"><?php echo $this->renderSection('footer'); ?></section>
  <section class="scripts"></section>

  <?php echo view('strut/vw_modal'); ?>
  ...
  <script>
    jQuery(window).on("load", function() {
      ...
      if (!executandoProcesso) {
        desBloqueiaTela();
      }
      msgtoast = '<?php echo esc(session('msg')); ?>';
      if (msgtoast != '') {
        setTimeout(mostranoToast(msgtoast), 750);
      }
    });
    // Exporta mensagens da sessão para JS
    // const msg_cfg = <?php echo json_encode(session('msg_cfg') ?? [], JSON_UNESCAPED_UNICODE); ?>;
  </script>
</body>
