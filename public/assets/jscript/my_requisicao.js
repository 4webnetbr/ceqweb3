...
async function carregarProdutos(url, aba, obj) {
  ...
  try {
    ...
    jQuery(".multiplica").on("change", function () {
      ...
      novomin = min * multiplicador;
      novomax = max * multiplicador;
      tr.attr("data-min", novomin);
      tr.attr("data-max", novomax);
    ...
    jQuery(".requisicao").on("change", function () {
      ...
      if (valAtual != 0) {
        ...
        const codproAtual = tr.getAttribute("data-codpro");
        let saldoDestinoAtual = 0;

        jQuery("tr").each(function () {
          const linha = jQuery(this)[0];
          if (linha.getAttribute("data-codpro") == codproAtual) {
            saldoDestinoAtual +=
              parseInt(linha.getAttribute("data-saldo-destino")) || 0;
          }
        });
