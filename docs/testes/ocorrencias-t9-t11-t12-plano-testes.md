...
| CT-T12-07 **[MIGR]** | RN03.18.1 | Status final segue "Alterar Status" configurado na origem | Migration aplicada; ação "Alterar Status" de origem configurada com `stt_id=X` | Executar a tratativa com essa ação | Ação de origem `stt_id=X` | Status final da ocorrência = `X` |
